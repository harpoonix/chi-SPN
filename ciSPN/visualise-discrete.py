...
for seed in all_seeds:
    ...
    if conf.explicit_load_part == "ciSPN":
        spn, _, _ = load_spn(num_condition_vars, load_dir=runtime_base_dir / "E1_CHC_ciSPN_NLLLoss" / "lr_schedule", discrete_ids=dataset.discrete_ids)
        eval_wrapper = spn
    ...
    with torch.no_grad():
        ...
        def compute_pdf(dim, y_dims, batch_size, low, high, interv_vector, cond_vars):
            ...
            joint = eval_wrapper.logpdf(x_batch, y_batch, marginalized)
            marginalized[:, dim] = 1.0
            support = eval_wrapper.logpdf(x_batch, y_batch, marginalized)
            values_x = (
            ...
            vals_pdf = np.exp((joint - support).cpu().numpy())
            print(f'values is {y_batch}')
            print(f'vals pdf is {vals_pdf}')
            exit(0);
        ...
        if conf.known_intervention:
            ...
            intervention_vector = np.unique(dataset.X.cpu().numpy(), axis=0)
