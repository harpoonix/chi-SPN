...
from ciSPN.E2_helpers import get_E2_loss_path, create_dataloader, create_cnn_model, create_cnn_for_spn, img_batch_processor
...
parser.add_argument("--provide_interventions", choices=["true", "false"], default="true")  # provide intervention vector during training and use intervention data
...
if __name__ == "__main__":
    ...
    if cli_args.dataset == "hiddenObject":
        dataset_split = "train"
        dataset_name = "hiddenObject" if conf.provide_interventions else "hiddenObjectCorrelation"
        hidden_object_base_dir = get_dataset_paths(dataset_name, dataset_split, get_base=True)
        dataset = HiddenObjectDataset(hidden_object_base_dir, split=dataset_split,
                                      add_intervention_channel=conf.provide_interventions, observational_only=not conf.provide_interventions)
    else:
    ...
    if conf.model_name == "ciCNNSPN":
        rg, params, spn = create_spn_model(
            num_target_vars, num_condition_vars, conf.seed,
            nn_provider=create_cnn_for_spn, nn_provider_args=nn_provider_args, setup="hiddenObject")
        model = spn
    ...
    # setup loss
    loss, loss_ispn = create_loss(conf.loss_name, conf, nn_provider=create_cnn_for_spn, nn_provider_args=nn_provider_args,
                                  load_dir=runtime_base_dir / get_E2_loss_path("hiddenObject", conf.loss_load_seed, provide_interventions=conf.provide_interventions))

    if conf.loss2_name is not None:
        loss2, loss2_ispn = create_loss(conf.loss2_name, conf, nn_provider=create_cnn_for_spn, nn_provider_args=nn_provider_args,
                                        load_dir=runtime_base_dir / get_E2_loss_path("hiddenObject", conf.loss_load_seed, provide_interventions=conf.provide_interventions))
