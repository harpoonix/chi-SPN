...
from ciSPN.E2_helpers import create_dataloader, create_cnn_for_spn, create_cnn_model, img_batch_processor
...
if __name__ == "__main__":
    ...
    dataloader = create_dataloader(dataset, conf.seed, num_workers=conf.num_workers, batch_size=conf.batch_size,
                                   multi_thread_data_loading=conf.multi_thread_data_loading, shuffle=False, drop_last=False)
    ...
    with torch.no_grad():
        ...
        rtpt = RTPT(name_initials=environment["runtime"]["initials"], experiment_name='CausalLossImg Eval', max_iterations=num_batches)
        rtpt.start()
        for batch_num, batch in enumerate(dataloader):
            condition_batch, target_batch = img_batch_processor(batch)

            if placeholder_target_batch is None or placeholder_target_batch.shape != batch["target"]:
                # zero out target vars to be sure
                placeholder_target_batch = torch.zeros_like(batch["target"], dtype=torch.float, device="cuda")
