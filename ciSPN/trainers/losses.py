    ...
    def forward(self, X, Y, prediction):
        ...
        with torch.no_grad():
            if self.marginals is None:
                # TODO assumes that the batch size does not change. (Passing a too large vector e.g. for last batch is ok.)
                # marginalize out all prediction vars
                self.marginals = torch.ones(prediction.shape, device=prediction.get_device(), requires_grad=False)
