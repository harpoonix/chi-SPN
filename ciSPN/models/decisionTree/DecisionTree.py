...
class DTNode:
    ...
    def print(self):
        print(("  "*self.level)+f"[{self.name}|{self._class}|{self.decision_feature}|{self.scores}]")
    ...
    def dot(self, dot, prefix="", name_delimiter="_", node_args=None, edge_args=None, content_func=None):
        ...
        if isinstance(content, collections.Mapping):
            node_content_name = content.get("name", None)
            if node_content_name is None:
                raise RuntimeError(f"Missing (or None) node name")
            node_args = {**node_args, **content}
            del node_args["name"]
        elif isinstance(content, str):
            node_content_name = content
        else:
            raise RuntimeError(f"unknown node content type: {type(content)}")
        dot.node(node_name, node_content_name, **node_args)

        child_names = [child.dot(dot, node_name+name_delimiter, name_delimiter, node_args, edge_args, content_func) for child in reversed(self.childs)]
    ...
    def prune(self):
        ...
        for child in self.childs:
            c_class = child.prune()
            # Even if we encounter a mixed child (c_class==None) we still iterate over the
            # other childs to be able to prune node inside them.

            # Check if mixed flag is not already set.
            if not is_mixed:
                if c_class is None:
                    # We found a mixed child node
                    is_mixed = True
                else:
                    if c_classes is None:
                        # this is the first time we encounter a pure node -> save the prediction
                        c_classes = c_class
                    else:
                        # check if both encountered prediction differ
...
class DecisionTree:
    ...
    def dot(self, graph_args, node_args, edge_args, content_func=None):
        ...
        self.get_root().dot(dot, node_args=node_args, edge_args=edge_args, content_func=content_func)
        return dot
