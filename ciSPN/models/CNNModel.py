...
def make_layers(cfg, batch_norm: bool = False, in_channels=3, inplace_relu=True):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d, torch.nn.BatchNorm2d(v), torch.nn.ReLU(inplace=inplace_relu)]
            else:
...
class SimpleCNNModelC(torch.nn.Module):
    ...
    def __init__(self, num_sum_weights, num_leaf_weights, inplace_relu=True, num_pools=6, num_channels=4):
        super().__init__()

        in_channels = num_channels  # (3 = rgb, 4 = rgb + intervention)

        last_layer_features = 64
        layers = [32, 'M', 48, 'M', 64, 64, 'M']

        self._cnn = VGGMod(make_layers(layers, batch_norm=True, in_channels=in_channels, inplace_relu=inplace_relu), num_pools=num_pools)
        ...
        self._leaf_head = torch.nn.Linear(num_intermediate_features, num_leaf_weights, bias=True)
        if num_sum_weights is None:
            self._sum_head = None
        else:
            self._sum_head = torch.nn.Linear(num_intermediate_features, num_sum_weights, bias=True)
