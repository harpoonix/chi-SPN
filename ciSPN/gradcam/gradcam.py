...
class GradCam:
    ...
    def generate_cam(self, input_image, target, partial_grad=None):
        # Full forward pass
        # conv_output is the output of convolutions at specified layer
        # model_output is the final output of the model (1, 1000)

        self.model.eval()
        conv_output, model_prediction = self.extractor.forward_pass(input_image, target.shape)
        ...
        if partial_grad is None:
            # do a full backwards pass with over all output variables
            model_prediction.backward(gradient=target, retain_graph=True)
        else:
            # do a partial backward pass from a single output
            model_prediction[:, partial_grad].backward(gradient=target[:, partial_grad], retain_graph=True)
        ...
        weights = np.mean(guided_gradients, axis=(1, 2))  # take average gradient over channels for each pixel
